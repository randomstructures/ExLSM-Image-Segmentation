...
def restore_mask_naive(float_mask):
    """Convert a interpolated mask (containing float / gray values) back to an integer mask where each integer encodes a class label.
        Naive apprach that rounds each pixel value to the closest int   

    Parameters
    ----------
    float_mask : image tensor
        corrupted mask

    Returns
    -------
    image tensor
        mask with rounded pixel values
    """
    r = np.round(float_mask)
    return r
...
def show_predictions(model, dataset, num=1):
    ...
    for image_batch, mask_batch in dataset:
        prediction_batch = model(image_batch) # evaluate the model on the batched input
        for image, mask, prediction in zip(image_batch, mask_batch, prediction_batch):
            if n < num:
...
def show_dataset(dataset, num=1):
    """Plot the image mask pairs deliv
    ...
    dataset : tf.Dataset
        Dataset of images and segmentation masks
    num : int, optional
        the number of pairs to show, by default 1
    """
    n = 0
    for image_batch, mask_batch in dataset:
...
def display(display_list, titles = True):
