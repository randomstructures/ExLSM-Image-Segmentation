...
class UnetTiler3D():
    ...
    def __init__(self, image, target, output_shape, input_shape):
        """
        Uses a pretrained 3D unet to segment an arbitraty input volume.

        A rectangular tiling of the unet output is extended to a collection of input regions. 
        The class handles reading and writing data from the h5 datasets that hold the image and segmentation mask.  

        Parameters
        ----------
        image : h5py dataset
            dataset containing the input tensor
        target : h5py dataset
            empty integer tensor where the segmentation mask is stored
        output_shape : tuple
...
# %%
test = UnetTiling3D(np.ones((10,20,30)), (5,5,5), (7,7,7))
t = test.getInputTile(6)
d = test._getSlice(t)
